...
通过 **QQ（OneBot/NapCat）、GitHub Issue/Discussion、CLI 终端**等多通道与用户互动，背后由统一 Skill 引擎（80+ Skill）、Multi-Agent 并行调度（30+ Worker）、三层记忆反思系统、独立代码审查闭环（Humanize/RLCR）、表情包智能管理系统驱动。
...
小白的一切能力都统一在 `SkillRegistry` 中（共 80+ 个）。启动时自动扫描注册，通过 JSON action 统一调用。

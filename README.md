...
OpenAkari-Codex 是一套以 **Humanize（RLCR 迭代审查循环）为核心工作流**的自主研究操作系统，以「**仓库即大脑**」为核心理念，让 LLM Agent 在无状态会话之间保持持久记忆。
...
- [Humanize RLCR — 核心工作流](#humanize-rlcr--核心工作流)
  - [RLCR 循环机制](#rlcr-循环机制)
  - [gen-plan 任务规划](#gen-plan-任务规划)
...
- [自主学习与经验积累](#自主学习与经验积累)
  - [三层记忆与反思](#三层记忆与反思)
  - [经验嵌入（compound）](#经验嵌入compound)
  - [事后分析（postmortem）](#事后分析postmortem)
  - [空闲探索（Fleet Idle）](#空闲探索fleet-idle)
  - [治理溯源与 ADR](#治理溯源与-adr)
- [高级功能](#高级功能)
  - [真实电话外呼](#真实电话外呼)
  - [全文件系统访问](#全文件系统访问)
...
| `ffmpeg` | 语音识别音频转换（STT） | `apt-get install ffmpeg` 或下载静态二进制 |
...
└────────────────────────┘  └──────────────────────────────┘
               ...
               └──────────┬──────────┘
                          ▼
       ┌──────────────────────────────────────┐
       │  Humanize RLCR 引擎 (核心工作流)      │
       │  gen-plan → 实现 → Self-Review → Fix  │
       │  → Post-Task 审查 (P0-P3 分级)        │
       │  Fleet 每任务自动执行, API fallback    │
       └──────────────────┬───────────────────┘
                          ▼
              ┌──────────────────────┐
              │  OpenAI API (GPT-5.4) │
...
│   ├── memory.py                   #   三层记忆系统
│   ├── memo.py                     #   备忘录与定时提醒
│   ├── media_tools.py              #   多媒体: TTS/Vision/STT/文件/CQ码/缓存清理
│   ├── sticker_manager.py          #   表情包: 收藏/标签/场景匹配/共享库
...
│   │   └── stickers/               #     表情包收藏(独立存储, 不受缓存清理)
│   ├── media_cache/                #   临时多媒体缓存(2天自动清理)
...
| **群聊 @** | @ 机器人 | role="public" 友好聊天 + 联网搜索等基础工具 |
| **好友请求** | 自动同意 | 通知主人设定人设 |
...
Humanize 是整个系统的质量引擎。无论是小白自身执行任务、还是 Fleet Worker 并行调度，**所有代码和文档产出都必须经过 Humanize RLCR 审查循环**。

### RLCR 循环机制

RLCR (Ralph-Loop with Codex Review) 是一个迭代式「实现-审查-修复」闭环：

```
gen-plan (结构化计划生成)
  ↓
Round 0: 实现代码/文档 (小白 / Fleet Worker)
  ↓
Self-Review: 实现者自检（对照 AC 验收标准）
  ↓
Codex Review: 独立 AI 审查 (P0-P4 分级问题)
  ↓
Round 1: 根据审查反馈修复
  ↓
Codex Review: 二次审查
  ↓
... 重复直到所有 AC 通过 或 达到最大迭代次数
```
...
| 阶段 | 动作 | 退出条件 |
|------|------|---------|
| Phase 1: 实现 | AI 实现 → 写摘要 → Codex 审查摘要 | Codex 输出 "COMPLETE" |
| Phase 2: 代码审查 | `codex review --base` 全量代码审查 → 标记 P0-P4 | 零问题或仅剩低优先级 |

**Goal Tracker 防止目标漂移**：
- **不可变区域**：终极目标 + AC 验收标准（Round 0 锁定，永不修改）
- **可变区域**：当前任务、已完成项、延期项、计划演化日志
- 每 5 轮执行一次全量目标对齐审计
...
这些计划文件存储在 `projects/<project>/plans/` 下，Worker 执行时严格对照 AC 进行。

### Fleet 强制审查

**每个 Fleet Worker 完成后，无论任务类型，都会自动执行 Humanize 审查**（`fleet/executor.py`）：

1. 如果 `codex` CLI 可用 → 使用 `codex review`（外部独立审查）
2. 否则 → **自动回退到 OpenAI API 直接审查**（读取变更文件 → LLM 审查 → 解析 P0-P3 问题）

| 审查级别 | 说明 | 后果 |
|---------|------|------|
| P0 (Critical) | 安全漏洞、数据丢失 | 任务标记 `blocked`，通知主人 |
| P1 (Major) | 逻辑错误、违反 AC | 任务标记 `blocked`，通知主人 |
| P2 (Minor) | 代码风格、性能 | 记录到溯源，不阻断 |
| P3 (Suggestion) | 建议优化 | 仅记录 |

审查结果写入 `governance.py` 的溯源系统（`ProvenanceRecord.humanize_reviews`）。
...
# 通过小白（QQ/CLI）
"审查一下 runner/agent_loop.py"          → humanize_review
"启动 RLCR 开发循环"                     → humanize_rlcr_setup
"Humanize 状态"                          → humanize_status
```
...
**核心审查与自我学习类**（以 Humanize 为中心）：

| Skill | 复杂度 | 说明 |
|-------|--------|------|
| `humanize` | high | **核心工作流** — RLCR 迭代开发 + gen-plan + Codex 独立审查 |
| `compound` | medium | **经验嵌入** — 将 session 发现固化为 conventions/skills/patterns（自我进化核心） |
| `compound-simple` | medium | 简化版经验嵌入（快速模型适用） |
| `postmortem` | opus-only | **事后分析** — Agent 推理失败根因追溯 + 预防措施 |
| `feedback` | high | 处理 PI/人类反馈 → 将反馈转化为持久改进 |
| `pua` | medium | 穷尽式问题解决（反复失败 2+ 次时自动触发） |
...
| `gravity` | medium | 基建判断: 重复模式 → 是否基建化 |
| `refresh-skills` | medium | Skill 同步: Skill 描述与源码对齐 |
...
| Skill | 说明 |
|-------|------|
| `media_pin` | 标记文件为"重要"，永久保留不被自动清理。参数: `filename` |
| `media_unpin` | 取消文件的"重要"标记 |
...
- `role="public"` — 普通用户：聊天 + 基础工具（联网搜索、网页抓取、时间查询、计算）
...
| 收到 | 处理方式 | 注入消息格式 |
|------|---------|-------------|
| 纯图片（表情包） | 自动收藏到表情包库 → 回复一个表情包 | 不描述图片内容，直接发表情 |
| 图片（有文字追问） | 自动下载 → Vision LLM 识别 | `[收到图片，内容识别: 这是一张...]` |
| 语音 | 自动下载 → Google STT / Whisper 转文字 | `[收到语音消息，识别内容: ...]` |
...
| 特性 | 说明 |
|------|------|
| **自动发送** | 每 2-3 条回复自动穿插一个表情包，根据对话场景选择合适的表情 |
| **收藏他人表情** | 收到其他用户发的纯图片表情包时自动收藏到 `logs/xiaobai/stickers/` |
| **共享库** | 所有用户发送的表情包统一共享，任何场景都可使用 |
| **Vision 标签** | 新收藏的表情包自动调用 Vision LLM 生成情绪/场景标签 |
| **收到表情回表情** | 收到纯图片/表情包时直接回复一个表情包，不会主动描述图片内容 |

表情包存储独立于媒体缓存，不受自动清理影响。
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `MEDIA_CACHE_TTL` | `172800`（2 天） | 缓存文件过期秒数，超期自动删除（表情包不受影响） |
| `MEDIA_CACHE_MAX_MB` | `500` | 缓存目录最大占用 MB，超出从最旧文件开始清理 |
| `WHISPER_API_KEY` | 空（可选） | OpenAI Whisper STT API Key。不填则使用免费 Google Web Speech API |
...
# STT（语音识别）—— 两种方案，优先免费的 Google Web Speech API
#   1. Google Web Speech API：无需任何 Key，需要 ffmpeg 将音频转为 FLAC
#      安装 ffmpeg: apt-get install ffmpeg
#      或下载静态二进制: https://johnvansickle.com/ffmpeg/
#   2. Whisper API（回退）：设置 WHISPER_API_KEY 环境变量
#      export WHISPER_API_KEY="your_whisper_key"

# Vision（图片识别）依赖 OpenAI 兼容 API（支持 vision 的模型）
```

**STT 不识别？** 检查 `ffmpeg` 是否在 PATH 中：`which ffmpeg`。如果网络受限（如 Clash 代理），Google STT 可能超时，此时会自动回退到 Whisper。

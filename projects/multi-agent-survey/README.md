...
- 2026-03-24T13:19:06Z — Completed the Coordination & Communication Phase 2 reading task. Added `projects/multi-agent-survey/literature/load-bearing-coordination-communication.md` to explicitly mark 7 load-bearing papers selected from the existing NeurIPS/ICML/ICLR/arXiv artifacts, then wrote one literature note for each: `multi-level-communication-note.md`, `language-grounded-communication-note.md`, `teammate-generation-note.md`, `hygma-note.md`, `cut-the-crap-note.md`, `benefits-limitations-communication-note.md`, and `goagent-note.md`. Provenance: task state updated in `projects/multi-agent-survey/TASKS.md`, session log in `projects/multi-agent-survey/logs/2026-03-24T131906Z-coordination-communication-load-bearing-notes.md`.
- 2026-03-24T21:18:53+08:00 — Wrote the first structured survey draft to `projects/multi-agent-survey/plans/survey-draft.md` using only in-repo literature inventories, load-bearing notes, and prior synthesis logs. The draft covers introduction, method taxonomy, key findings, trend analysis, and future directions. Provenance: `wc -m projects/multi-agent-survey/plans/survey-draft.md` returned `17206`, exceeding the Phase 3 task requirement of `≥3000 字`; task state updated in `projects/multi-agent-survey/TASKS.md`, session log in `projects/multi-agent-survey/logs/2026-03-24T211853+0800-survey-draft.md`.
- 2026-03-24T21:33:47+08:00 — Re-answered the open question on recent arXiv subdirections using only existing in-repo evidence and recorded the answer here. Provenance: `projects/multi-agent-survey/literature/arxiv-2026-01-to-2026-03.md` reports 396 deduplicated records in the 2026-01-01 to 2026-03-23 window and classification counts across the 32-paper selected subset of `Architecture = 28`, `Evaluation = 21`, `Theory/MARL = 15`, `Coordination = 14`, `Communication = 8`, `Application = 8`, `Theory = 4`; `projects/multi-agent-survey/logs/2026-03-23T16:40:00Z-arxiv-subdirection-synthesis.md` synthesizes these counts into three dominant subdirections: (1) Architecture / Orchestration as the strongest line, (2) Evaluation-heavy agent systems as the second line, and (3) MARL-backed coordination as a still-strong but increasingly blended line. Repo-internal answer: recent arXiv multi-agent papers are concentrated mainly in architecture/orchestration, then evaluation-centric agent systems, with MARL-based coordination remaining important; communication appears more often as a component of architecture/coordination than as a standalone dominant cluster. No new external research was needed for this answer; the remaining research need is to identify which representative papers in these three subdirections are load-bearing for the final survey.
- 2026-03-24T21:34:46+08:00 — Horizon scan of the existing repository found one genuinely new ICLR 2026 benchmark direction not yet captured as a standalone note: `projects/multi-agent-survey/literature/liveresearchbench-note.md` for **LiveResearchBench: Benchmarking Single- and Multi-Agent Systems for Citation-Grounded Deep Research**. Provenance: `search_text("A2ASecBench|LiveResearchBench|Collaborative Gym|DoVer|EduVisAgent|SocialJax", "projects/multi-agent-survey", max_results=50)` returned no prior matches before the note was written; the paper is listed in `projects/multi-agent-survey/literature/iclr-2025-2026.md`, entry 103, as an ICLR 2026 Poster tagged `Architecture, Evaluation, Application`. This is the strongest current in-repo benchmark candidate for the open question about the boundary between single-agent and multi-agent systems. Caveat: direct OpenReview abstract extraction remains blocked by HTTP 403 in this environment, so the note is provisional rather than a full deep-reading note.
- 2026-03-24T21:40:10+08:00 — Added `projects/multi-agent-survey/analysis/research-gaps.md` to identify 7 concrete research gaps and future directions using only in-repo artifacts. The new analysis argues that the largest unresolved gaps are (1) matched-budget single-vs-multi boundary studies, (2) coordination cost models, (3) topology-learning regularities, (4) deployment-grade evaluation, (5) safety-by-design for cascading failures, (6) human-AI mixed-agent evaluation standards, and (7) architecture-specific load-bearing synthesis. Provenance: the analysis cites only existing repo artifacts including `analysis/2026-03-24-trends-2024-to-2026.md`, `analysis/2026-03-24-method-taxonomy-and-comparison.md`, `logs/2026-03-23T170500Z-single-vs-multi-agent-boundary-synthesis.md`, `literature/arxiv-2026-01-to-2026-03.md`, `literature/iclr-2025-2026.md`, and the current evaluation / coordination literature notes. Task state updated in `projects/multi-agent-survey/TASKS.md`.
...
- 在最近三个月 arXiv 的三条主线中，哪些论文应被视为 load-bearing 代表作（Architecture / Orchestration、Evaluation-heavy systems、MARL-backed coordination）？

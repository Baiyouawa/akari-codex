...
- 完成“将这 10 篇综述论文下载到本地，按统一目录结构整理 PDF 与元数据，确保文件可追溯到原始来源”任务。
- 在 `projects/multi-agent-survey-review/literature/papers/` 下按 `paper_id` 为 10 篇最终综述建立统一归档目录；每篇均包含 `paper.pdf`、`metadata.json`、`source.txt`、`download.log`、`checksums.txt`。
- 新增批量索引：`projects/multi-agent-survey-review/literature/indexes/papers_index.jsonl` 与 `projects/multi-agent-survey-review/literature/indexes/papers_index.csv`；新增整理日志 `projects/multi-agent-survey-review/literature/logs/run_2026-03-26T23-40-21+08-00.log` 与摘要 `projects/multi-agent-survey-review/literature/meta/2026-03-26-ingest-summary.json`。
- 元数据来源：8 篇 arXiv 条目通过 arXiv API 补齐作者、摘要、分类与发布时间；2 篇 Springer/DOI 条目保留 DOI、publisher version、落地页与 PDF 直链，并统一写入哈希、页数、来源类型、访问时间和 provenance 字段。
- 自审结果：无 P0/P1；并将 `TASKS.md` 中该任务标记为已完成。
